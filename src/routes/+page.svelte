	...
	<div class="w-full max-w-2xl flex flex-col gap-6">
		...
		<div class="flex flex-col gap-6 bg-[--color-panel] border border-[--color-border] rounded-2xl p-7">
			...
			<div class="flex flex-col gap-1.5">
				<label for="topic" class="text-[11px] font-semibold uppercase tracking-widest text-[--color-muted]">Topic</label>
			...
			<div class="grid grid-cols-[1fr_36px_1fr] items-end gap-2">
				<div class="flex flex-col gap-1.5">
					<label for="agentA" class="text-[11px] font-semibold uppercase tracking-widest flex items-center gap-1.5" style="color: {getModelInfo(agentA).color}">
						<span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background: {getModelInfo(agentA).color}"></span>
						Agent A
					</label>
					<select
						id="agentA"
				...
				<div class="flex flex-col gap-1.5">
					<label for="agentB" class="text-[11px] font-semibold uppercase tracking-widest flex items-center gap-1.5 justify-end" style="color: {getModelInfo(agentB).color}">
						Agent B
						<span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background: {getModelInfo(agentB).color}"></span>
					</label>
					<select
						id="agentB"
						bind:value={agentB}
			...
			<div class="flex items-center gap-3 flex-wrap">
				<div class="flex items-center gap-2 bg-[--color-surface] border border-[--color-border] rounded-xl px-3 py-2">
					<label for="turns" class="text-[11px] font-semibold uppercase tracking-widest text-[--color-muted] whitespace-nowrap">Turns</label>
					<input
						id="turns"
						type="number"
						bind:value={turns}
				...
				>
					<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15.172 7l-6.586 6.586a2 2 0 1 0 2.828 2.828l6.414-6.586a4 4 0 0 0-5.656-5.656l-6.415 6.585a6 6 0 1 0 8.486 8.486L20.5 13" />
					</svg>
					Files {#if contextFiles.length > 0}<span class="ml-0.5 bg-[--color-accent] text-white rounded-full px-1.5 py-0 text-[10px] leading-4">{contextFiles.length}</span>{/if}
				</button>
			...
			{#if showFiles}
				<div class="flex flex-col gap-2 pt-1 border-t border-[--color-border-subtle]">
					<!-- svelte-ignore a11y_no_static_element_interactions -->
					<label
						for="file-input"
						class="flex items-center justify-center gap-2.5 border border-dashed rounded-xl px-5 py-3.5 cursor-pointer transition-all
							{dragging ? 'border-[--color-accent] bg-[#7c6af7]/5' : 'border-[--color-border] hover:border-[--color-muted]'}"
					...
					>
						...
						<span class="text-sm text-[--color-muted-fg]">Drop files or <span class="text-[--color-accent]">browse</span></span>
					...
					{#if contextFiles.length > 0}
						<div class="flex flex-wrap gap-2">
							{#each contextFiles as file (file.name)}
								<div class="flex items-center gap-2 bg-[--color-surface] border border-[--color-border] rounded-lg pl-3 pr-2 py-1.5">
									<svg class="w-3 h-3 text-[--color-muted] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									...
									<span class="text-xs text-[--color-muted-fg] max-w-[140px] truncate">{file.name}</span>
									<span class="text-[10px] text-[--color-muted]">{(file.content.length / 1024).toFixed(1)}KB</span>
									<button onclick={() => removeFile(file.name)} disabled={running} class="text-[--color-muted] hover:text-red-400 transition-colors disabled:opacity-40 cursor-pointer ml-0.5" aria-label="Remove {file.name}">
		...
		<!-- Chat -->
		<div class="flex flex-col rounded-2xl border border-[--color-border] overflow-hidden bg-[--color-panel]">
			...
			{#if running || (done && messages.length > 0)}
				<div class="flex items-center gap-3 px-6 py-3 border-b border-[--color-border-subtle] bg-[--color-panel]/80 backdrop-blur-sm sticky top-0 z-10">
					...
					<span class="text-[11px] font-semibold tabular-nums text-[--color-muted] whitespace-nowrap">{messages.length}<span class="text-[--color-border] mx-0.5">/</span>{turns}</span>
					{#if done}
						<span class="text-[11px] uppercase tracking-widest text-[--color-muted]">done</span>
					{/if}
				</div>
			{/if}
			...
			>
						...
						<div class="flex items-center gap-8">
							<div class="flex flex-col items-center gap-2.5">
								<div class="w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold shadow-lg"
									style="background: linear-gradient(135deg, {getModelInfo(agentA).color}22, {getModelInfo(agentA).color}08); color: {getModelInfo(agentA).color}; border: 1px solid {getModelInfo(agentA).color}30">
									{getModelInfo(agentA).name[0]}
								</div>
								<span class="text-xs font-semibold text-center max-w-[100px] leading-snug" style="color: {getModelInfo(agentA).color}">{getModelInfo(agentA).name}</span>
							</div>

							<div class="flex flex-col items-center gap-1">
								<span class="text-[11px] font-bold uppercase tracking-[0.4em] text-[--color-muted]">vs</span>
							...
							<div class="flex flex-col items-center gap-2.5">
								...
								<span class="text-xs font-semibold text-center max-w-[100px] leading-snug" style="color: {getModelInfo(agentB).color}">{getModelInfo(agentB).name}</span>
							</div>
						</div>
						{#if topic.trim()}
							<div class="max-w-sm text-center">
								<p class="text-xs uppercase tracking-widest text-[--color-muted] mb-2">Topic</p>
								<p class="text-base text-[--color-muted-fg] font-medium leading-snug">"{topic}"</p>
							</div>
						{/if}
						<p class="text-sm text-[--color-muted] text-center">Hit <span class="text-white font-semibold">Start debate</span> to begin · Ctrl+Enter</p>
					</div>
				{/if}
				...
				{#if running}
					{#if streamingMessage}
						...
						>
							<div class="flex-shrink-0 flex flex-col items-center gap-1.5">
								<div class="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold ring-2 ring-offset-1 ring-offset-[--color-panel] transition-all"
								...
								<span class="text-[10px] tabular-nums text-[--color-muted]">#{messages.length + 1}</span>
							</div>
							<div class="flex flex-col gap-1.5 min-w-0 {isLeft ? 'items-start' : 'items-end'}" style="max-width: calc(100% - 3rem)">
								<span class="text-[10px] font-bold uppercase tracking-widest" style="color: {streamingMessage.color}">{streamingMessage.agentName}</span>
								<p class="text-[14.5px] leading-relaxed text-[#d4d4e8] px-4 py-3.5 rounded-2xl {isLeft ? 'rounded-tl-sm border-l-2' : 'rounded-tr-sm border-r-2'}"
									style="background-color: {streamingMessage.color}09; border-color: {streamingMessage.color}30; border-top: 1px solid {streamingMessage.color}1a; border-bottom: 1px solid {streamingMessage.color}1a; {isLeft ? '' : 'border-left: 1px solid ' + streamingMessage.color + '1a'}">{streamingMessage.text}<span class="inline-block w-[2px] h-[1em] ml-[1px] align-text-bottom rounded-sm animate-pulse" style="background: {streamingMessage.color}"></span></p>
							</div>
						</div>
					{:else}
						<div class="flex items-center gap-3 px-6 py-4 {messages.length > 0 ? 'border-t border-[--color-border-subtle]' : ''}">
							...
							<div class="flex gap-1 items-center">
								<span class="w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:0ms]" style="background: {typingAgentColor || 'var(--color-muted)'}"></span>
								<span class="w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:160ms]" style="background: {typingAgentColor || 'var(--color-muted)'}"></span>
								<span class="w-1.5 h-1.5 rounded-full animate-bounce [animation-delay:320ms]" style="background: {typingAgentColor || 'var(--color-muted)'}"></span>
							</div>
							{#if typingAgentName}
								<span class="text-xs text-[--color-muted]" style="color: {typingAgentColor}AA">{typingAgentName} is thinking…</span>
							{/if}
						</div>
					{/if}
				{/if}
		...
		{#if messages.length > 0}
			<div class="flex items-center gap-2">
				<span class="text-[11px] font-semibold uppercase tracking-widest text-[--color-muted] mr-1">Export</span>
				<button onclick={() => exportDebate('md')}
					...
					<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-1m-4-4-4 4m0 0-4-4m4 4V4" /></svg>
					Markdown
				</button>
