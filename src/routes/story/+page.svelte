<script lang="ts">
	...
	let progress = $derived(rounds > 0 ? Math.min((paragraphs.length / (rounds * 3)) * 100, 100) : 0);
	...
	function exportStory(format: 'md' | 'txt') {
		...
		const safeTitle = premise.slice(0, 40).replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'story';
		...
		if (format === 'md') {
			content = `# Story\n\n**Premise:** ${premise}\n\n_Exported ${date}_\n\n---\n\n`;
			content += paragraphs.map((p) => `_${p.agentName}:_\n${p.text}`).join('\n\n');
			mime = 'text/markdown';
			ext = 'md';
		} else {
			content = `STORY\nPremise: ${premise}\nExported: ${date}\n${'─'.repeat(40)}\n\n`;
			content += paragraphs.map((p) => `[${p.agentName}]\n${p.text}`).join('\n\n');
			mime = 'text/plain';
			ext = 'txt';
		}
	...
	function onPremiseKeydown(e: KeyboardEvent) {
		if (e.key === 'Enter' && (e.ctrlKey || e.metaKey) && !running && premise.trim()) {
			e.preventDefault();
			startStory();
		}
	}
				...
				for (const part of parts) {
					...
					if (data.type === 'token') {
						if (!streamingParagraph) {
							streamingParagraph = { agentId: data.agentId, agentName: data.agentName, color: data.color, text: data.text };
							typingAgentName = '';
							typingAgentColor = '';
						} else {
							streamingParagraph = { agentId: streamingParagraph.agentId, agentName: streamingParagraph.agentName, color: streamingParagraph.color, text: streamingParagraph.text + data.text };
						}
	...
	<div class="w-full max-w-3xl flex flex-col gap-6">
		...
		<header class="text-center flex flex-col items-center gap-2">
			<div class="relative">
				...
				<h1 class="relative font-display text-5xl sm:text-6xl font-bold tracking-tight">
					<span class="text-white">ai</span><span class="text-transparent bg-clip-text bg-gradient-to-r from-[#34d399] to-[#059669]">story</span>
				</h1>
			</div>
			<p class="text-sm text-[--color-muted] tracking-wide">collaborative fiction by language models</p>
			<!-- Nav -->
		...
		<div class="flex flex-col gap-6 bg-[--color-panel] border border-[--color-border] rounded-2xl p-7">
			...
			<div class="flex flex-col gap-1.5">
				<label for="premise" class="text-[11px] font-semibold uppercase tracking-widest text-[--color-muted]">Premise</label>
				<textarea
					id="premise"
					bind:value={premise}
			...
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
				{#each [{id: 'agentA', bind: () => agentA, set: (v: string) => agentA = v, label: 'Author 1'}, {id: 'agentB', bind: () => agentB, set: (v: string) => agentB = v, label: 'Author 2'}, {id: 'agentC', bind: () => agentC, set: (v: string) => agentC = v, label: 'Author 3'}] as agentConf}
					<div class="flex flex-col gap-1.5">
						<label for={agentConf.id} class="text-[11px] font-semibold uppercase tracking-widest flex items-center gap-1.5" style="color: {getModelInfo(agentConf.bind()).color}">
							<span class="w-1.5 h-1.5 rounded-full flex-shrink-0" style="background: {getModelInfo(agentConf.bind()).color}"></span>
							{agentConf.label}
						</label>
						<select
							id={agentConf.id}
							...
							style="border-left: 2px solid {getModelInfo(agentConf.bind()).color}"
						>
			...
			<div class="flex items-center gap-3 flex-wrap">
				<div class="flex items-center gap-2 bg-[--color-surface] border border-[--color-border] rounded-xl px-3 py-2">
					<label for="rounds" class="text-[11px] font-semibold uppercase tracking-widest text-[--color-muted] whitespace-nowrap">Rounds</label>
					<input
						id="rounds"
						type="number"
						bind:value={rounds}
				...
				{:else}
					<button
						type="button"
						onclick={() => { if (!running && premise.trim()) startStory(); }}
		...
		<div class="flex flex-col rounded-2xl border border-[--color-border] overflow-hidden bg-[--color-panel]">
			...
			{#if running || (done && paragraphs.length > 0)}
				<div class="flex items-center gap-3 px-6 py-3 border-b border-[--color-border-subtle] bg-[--color-panel]/80 backdrop-blur-sm sticky top-0 z-10">
					<div class="flex-1 h-1 bg-[--color-border] rounded-full overflow-hidden">
						<div class="h-full rounded-full transition-all duration-500" style="width: {progress}%; background: linear-gradient(to right, #34d399, #059669)"></div>
					</div>
					<span class="text-[11px] font-semibold tabular-nums text-[--color-muted] whitespace-nowrap">{paragraphs.length}<span class="text-[--color-border] mx-0.5">/</span>{rounds * 3}</span>
					{#if done}
			...
			>
				...
				{#if paragraphs.length === 0 && !running && !streamingParagraph}
					<div class="flex flex-col items-center justify-center gap-6 flex-1 py-10">
						<div class="text-4xl">📖</div>
						<p class="text-sm text-[--color-muted] text-center max-w-sm">
							Hit <span class="text-white font-semibold">Start Story</span> to begin writing. The AI models will take turns writing the next paragraph.
						</p>
					</div>
				{/if}

				{#if paragraphs.length > 0 || streamingParagraph}
					<p class="mb-6 font-serif italic text-[#a1a1aa] border-l-4 border-[#34d399] pl-4">{premise}</p>
				{/if}

				<!-- Paragraphs -->
				{#each paragraphs as p, i (i)}
					<div class="mb-6 group relative" style="animation: fadeSlide 0.2s ease both; animation-delay: {Math.min(i * 20, 100)}ms">
						<div class="absolute -left-6 md:-left-10 top-1 opacity-0 group-hover:opacity-100 transition-opacity">
							<div class="w-4 h-4 rounded-full flex items-center justify-center" style="background: {p.color}40; border: 1px solid {p.color}" title={p.agentName}></div>
						</div>
						<p class="font-serif text-[17px] md:text-[19px] leading-[1.8] tracking-wide" style="color: {i === paragraphs.length - 1 && !running ? '#ffffff' : '#e2e8f0'}">
							{p.text}
						</p>
						<div class="mt-1 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
							<span class="text-[9px] font-bold uppercase tracking-widest" style="color: {p.color}">{p.agentName}</span>
						</div>
					</div>
				{/each}
				...
				{#if running}
					{#if streamingParagraph}
						<div class="mb-6 relative" style="animation: fadeSlide 0.15s ease both">
							<div class="absolute -left-6 md:-left-10 top-1">
								<div class="w-4 h-4 rounded-full flex items-center justify-center animate-pulse" style="background: {streamingParagraph.color}80; border: 1px solid {streamingParagraph.color}" title={streamingParagraph.agentName}></div>
							</div>
							<p class="font-serif text-[17px] md:text-[19px] leading-[1.8] tracking-wide text-white">
								{streamingParagraph.text}<span class="inline-block w-[3px] h-[1em] ml-[2px] align-text-bottom rounded-sm animate-pulse" style="background: {streamingParagraph.color}"></span>
							</p>
							<div class="mt-1 flex items-center gap-1.5">
								<span class="text-[9px] font-bold uppercase tracking-widest" style="color: {streamingParagraph.color}">{streamingParagraph.agentName}</span>
							</div>
						</div>
					{:else if typingAgentName}
						<div class="flex items-center gap-3 py-4">
							<div class="w-5 h-5 rounded-md flex items-center justify-center text-[9px] font-bold flex-shrink-0 ring-1"
								style="background: {typingAgentColor}15; color: {typingAgentColor}; ring-color: {typingAgentColor}30">
								{typingAgentName[0]}
							</div>
							<div class="flex gap-1 items-center">
								<span class="w-1 h-1 rounded-full animate-bounce [animation-delay:0ms]" style="background: {typingAgentColor || 'var(--color-muted)'}"></span>
								<span class="w-1 h-1 rounded-full animate-bounce [animation-delay:160ms]" style="background: {typingAgentColor || 'var(--color-muted)'}"></span>
								<span class="w-1 h-1 rounded-full animate-bounce [animation-delay:320ms]" style="background: {typingAgentColor || 'var(--color-muted)'}"></span>
							</div>
							<span class="text-xs text-[--color-muted]" style="color: {typingAgentColor}AA">{typingAgentName} is writing…</span>
						</div>
					{/if}
				{/if}

				{#if done}
					<div class="flex items-center gap-3 py-6 mt-4 border-t border-[--color-border-subtle]">
		...
		{#if paragraphs.length > 0}
			<div class="flex items-center gap-2">
				...
				<button onclick={() => exportStory('txt')}
					class="flex items-center gap-1.5 bg-[--color-panel] border border-[--color-border] hover:border-[#34d399] hover:text-white text-[--color-muted-fg] text-xs font-medium px-3.5 py-2 rounded-lg transition-all cursor-pointer">
					<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-1m-4-4-4 4m0 0-4-4m4 4V4" /></svg>
					Plain text
				</button>
