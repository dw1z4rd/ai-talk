...
async function loadInitialState() {
  ...
  try {
    ...
    const drops = logs.filter(
      (l: any) => l.event === "connection_dropped" || l.event === "bomb_served",
    );
    drops.sort(
      (a: any, b: any) =>
        parseFloat(b.duration_seconds ?? 0) -
        parseFloat(a.duration_seconds ?? 0),
    );
    totalWasted = drops.reduce(
      (acc: number, l: any) => acc + parseFloat(l.duration_seconds ?? 0),
      0,
    );
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (!VALID_MODES.includes(mode)) {
    return new Response(
      JSON.stringify({ error: "Invalid mode. Must be: random, llm, or bomb" }),
      {
        status: 400,
...
export const GET: RequestHandler = ({ request }) => {
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const onBotDetected = (payload: unknown) =>
        safeSend({ type: "bot_detected", ...(payload as object) });
      const onBombServed = (payload: unknown) =>
        safeSend({ type: "bomb_served", ...(payload as object) });
      const onTarpitChunk = (payload: unknown) =>
        safeSend({ type: "tarpit_chunk", ...(payload as object) });
      const onBombReady = (payload: unknown) =>
        safeSend({ type: "bomb_ready", ...(payload as object) });
      const onModeChanged = (payload: unknown) =>
        safeSend({ type: "mode_changed", ...(payload as object) });
