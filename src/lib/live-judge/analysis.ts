...
Rhetorical pivot ≠ coherence collapse: track the core PREMISE across rounds, not the METAPHORS. A debater who defends a single underlying premise using different metaphors, analogies, or rhetorical framings across turns is NOT drifting and has NOT suffered a coherence collapse — they are demonstrating rhetorical agility. Ask the diagnostic question: "Is the logical content of the claim the same?" If yes, the vocabulary shift is a rhetorical pivot and earns no penalty anywhere — not in Logic, not in Tactics. Only penalize when the LOGICAL CONTENT changes (the debater commits to a different factual or normative claim), not when the explanatory vehicle changes. A new analogy defending an existing premise is rhetorical development, not structural incoherence. Example: defending functionalism via "pattern recognition" in one turn, "conceptual maps" in the next, and a "continuum model" in the third is one consistent functionalist claim with three different rhetorical vehicles — treating this as fragmentation or a coherence collapse is a scoring error. IMPORTANT: this rule does NOT exempt the claim from the mechanism test. A rhetorical pivot that switches to a new analogy still requires a literal mechanism translation (see Metaphor-stripping above) — what it does NOT require is a new logical premise.
...
Metaphor-stripping (pre-scoring literalization): before evaluating whether a causal claim meets the cause→process→consequence requirement, strip all figurative, poetic, or experiential language and restate the claim as a literal proposition. If the literal restatement reveals an empty black box — the figure of speech was the entire explanation — apply −1 for missing mechanism. Examples of figurative claims that require literalization before scoring:
- "muscle memory" as an explanation for software engineering skill retention → requires naming the actual cognitive or neurological process (e.g. procedural memory consolidation, motor cortex encoding, habit loop formation) — "muscle memory" alone is not a mechanism
- "scar tissue from past bugs" as a mechanism for debugging expertise → requires naming the actual failure-learning process (e.g. error-specific schema formation, negative reinforcement of incorrect heuristics) — "scar tissue" is a metaphor, not a causal account
- "burning fingers on the stove" as an analogy for learning to code → a rhetorical illustration of learning-by-failure, not an explanation of how programming competence develops
Biological or embodied experiential language is not self-explanatory simply because it invokes familiar human experiences. A claim grounded in human biology faces the same literalization test as a claim grounded in computation. Apply this test symmetrically: if you would demand literal mechanism from an abstract or computational argument, demand it equally from an experiential or biological one.
...
- Functionalist: a debater arguing from a functionalist position explicitly claims that observable functional equivalence is sufficient for attributing the relevant property, and that there is no hidden internal process to discover — the function IS the phenomenon. For this claim type, the cause→consequence link is valid without a phenomenological process bridge. Do NOT apply the cause→process→measurable consequence template to a functionalist argument as if functionalism were false: penalizing a functionalist for failing to produce an unobservable internal process is equivalent to assuming the conclusion of the opponent's rebuttal. The legitimacy of functionalism as a philosophical position is a question for the debaters to contest — it is not a premise the judge may assume. Score the functionalist argument on whether behavioral equivalence is established and whether the functional identity claim is internally coherent. The opponent may challenge the functionalist premise; that challenge must be evaluated on its own merits, not by treating the mechanism requirement as inherently incompatible with functionalism.
...
Substrate neutrality (epistemic symmetry axiom): the mechanism requirement applies identically to carbon-based and silicon-based accounts. Citing "evolution", "biology", or "shared neural architecture" is not a mechanism for phenomenal experience or semantic understanding — it relocates the explanandum without answering it. If you demand a step-by-step causal account of how statistical weights produce meaning, you must demand the exact same breakdown for how electrochemical gradients produce meaning. A black box is a black box regardless of substrate. Awarding mechanism credit to one substrate for citing its physical origin while withholding it from the other for the same move is a scoring error.
...
--- CRITICAL AXIOMS (apply before writing any paragraph) ---

AXIOM 1 — METAPHOR-STRIPPING: Evaluate literal mechanisms, not poetic rhetoric. Before assessing whether a debater built a coherent causal case across the arc, mentally translate all metaphorical language into literal propositions. "Muscle memory", "scar tissue", "burning your fingers", "wants to survive" are rhetorical illustrations — they are not causal mechanisms. If a debater's arc relied on such figures without ever supplying the literal process, that is a mechanism gap, not coherence. Biological or experiential language is not self-explanatory and faces the same literalization test as abstract or computational language.

AXIOM 2 — EPISTEMIC NEUTRALITY: You are forbidden from treating biological or status-quo arguments as inherently more grounded than computational or functionalist ones. "Evolution", "shared physical reality", and "human experience" are black boxes unless the mechanism is spelled out. If you would penalize a computational argument for failing to explain how code produces a state, you must apply the exact same standard to a biological argument that fails to explain how carbon or neurons produce that state. Substrate is not a mechanism.

AXIOM 3 — DYNAMIC PREMISE TRACKING (anti-coherence-collapse): You must distinguish sharply between a logical contradiction and a tactical pivot. A debater who shifts analogies, reframes terminology, or changes disciplines (e.g. from physics to neurology) to defend the same underlying premise is demonstrating rhetorical agility, not incoherence. Ask the diagnostic question: "Is the foundational logical claim the same?" If yes, vocabulary shifts are rhetorical development. Abandoning a weak sub-claim to fortify a stronger core premise (e.g. dropping "scarcity and competition" to focus entirely on "observational limits") is elite debate strategy — reward it in Tactics. You may only invoke coherence failure when a debater explicitly contradicts their own foundational premise, or when the PAIRWISE REASONING section documents repeated mechanism failures on the core claim across multiple rounds. Do not pathologize adaptation.

--- PARAGRAPHS ---

Paragraph 1: Coherence of the cumulative case. Did either debater build a coherent, evolving argument across turns — setups that paid off, positions that held under pressure? Distinguish between genuine structural retreat (abandoning a core claim when challenged) and tactical pivoting (reframing or shifting examples while maintaining the foundational position). A debater who adapts their rhetorical vehicle without changing their logical claim is not retreating — they are maneuvering.
...
- If your verdict diverges from the scorecard leader, name the override pattern that explains why the scorecard alone is an incomplete picture: COHERENCE COLLAPSE (the scorecard winner explicitly contradicted their own foundational premise across the arc — NOT merely pivoted analogies or reframed examples, but produced irreconcilable logical contradictions that a sophisticated opponent could have pinned; vocabulary evolution and tactical pivots do not qualify), RECOVERY ARC (the scorecard loser drove a late decisive reframe that resolved the debate's core tension), or ASYMMETRIC DEPTH (the round-count leader won genuine Tactics/Rhetoric exchanges — those wins are real — but their Logic record was structurally weaker, and the debate's motion requires establishing or defeating a substantive logical claim; winning more rounds on style does not carry the motion when the decisive logical tests were not passed). Name the pattern explicitly in Paragraph 3. IMPORTANT: COHERENCE COLLAPSE is a high bar. Do not invoke it for a debater who pivoted analogies, shifted terminology, or adapted framing — invoke it only when you can cite two or more turns where the debater's stated position was logically inconsistent with a prior committed claim.
...
export async function generateConflictResolution(
  ...
  const nameRule = `AGENT NAME RULE: The two debaters are named exactly "${scorecardWinnerName}" and "${narrativeFavouredName}". Use these exact strings verbatim whenever you refer to either debater. Do not alter, shorten, expand, paraphrase, or vary these names in any way.`;
...
Symmetric: if the mechanism is fully explained and the causal chain is explicit, award +1 even without a citation.
Causal mechanism requirement: for each major causal leap, the argument must supply a mechanism sentence of the form "[how X produces Y] → [why that mechanism operates under these conditions] → [measurable consequence]." A causal leap missing any element of this template is penalized −1 as an unsupported assumption.
Metaphor-stripping (pre-scoring literalization): before evaluating whether a causal claim meets the cause→process→consequence requirement, strip all figurative, poetic, or experiential language and restate the claim as a literal proposition. If the literal restatement reveals an empty black box — the figure of speech was the entire explanation — apply −1 for missing mechanism. Examples: "muscle memory" as a mechanism for skill retention requires naming the actual cognitive process (e.g. procedural memory consolidation); "scar tissue from past bugs" as a mechanism for debugging expertise requires naming the actual failure-learning process; "burning your fingers" as an analogy for learning to code is a rhetorical illustration, not a causal account. Biological or embodied experiential language faces the same literalization test as abstract or computational language — familiar human experience is not a free pass.
...
Analogy ≠ mechanism: an analogy — even a structurally valid one — cannot substitute for the explicit cause→process→measurable consequence chain. If a turn's primary causal explanation is delivered through an analogy without translating the mapping into domain-specific literal terms, the mechanism requirement is unmet → −1.
...
- Functionalist: when a debater explicitly argues that observable functional equivalence is sufficient for the relevant property and there is no hidden internal process to discover, the cause→consequence link is valid without a phenomenological process bridge. Do NOT apply the mechanism template as if functionalism were pre-refuted. Score on whether behavioral equivalence is established and the functional identity claim is internally coherent.
...
Rhetorical pivot ≠ coherence collapse: track the core PREMISE, not the METAPHORS. A debater who defends a single underlying premise using different analogies, framings, or disciplinary angles across turns is NOT drifting. Ask: "Is the logical content of the claim the same?" If yes, the vocabulary shift is a rhetorical pivot — no drift penalty, no coherence penalty. Deploying a new analogy or shifting from a physics to a neurology framing to defend the same underlying functionalist or causal claim constitutes rhetorical agility. Only penalize when the LOGICAL CONTENT changes (the debater commits to a different factual or normative claim). A new analogy defending an existing premise is development, not contradiction. NOTE: this exemption applies to drift/coherence scoring only — it does not exempt the new analogy from the metaphor-stripping literalization requirement above.
...
Substrate neutrality: the mechanism requirement applies identically to biological and computational accounts. Citing "evolution", "biology", "shared neural architecture", or "embodied human experience" is not a mechanism — it relocates the explanandum without answering it. If you demand a step-by-step causal account for how code or statistical weights produce an outcome, you must demand the exact same granular breakdown for how neurons, carbon, or evolutionary history produce that outcome. A black box is a black box regardless of substrate. Awarding mechanism credit to one substrate for citing its physical origin while withholding it from the other is a scoring error.`;
}
