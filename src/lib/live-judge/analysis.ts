...
export async function generateConflictResolution(
  ...
  const patternDefinitions: Record<string, string> = {
    ...
    "ASYMMETRIC DEPTH":
      "one debater accumulated genuine Tactics/Rhetoric wins — which are real turn-level advantages, not hollow — but the debate's motion requires establishing or defeating a positive substantive logical claim; winning exchanges on style does not carry the motion if the logical burden was never met. Scoring more rounds does not equal winning the debate when the decisive logical tests were not passed",
  };
  ...
  if (claimedOverridePattern) {
    ...
    const asymmetricDepthNote =
      claimedOverridePattern === "ASYMMETRIC DEPTH"
        ? ` IMPORTANT for ASYMMETRIC DEPTH: the Tactics and Rhetoric wins by the round-count leader are real wins — ASYMMETRIC DEPTH is not a claim that those wins were fake or hollow. It is a claim that the dimension mix means the round-count leader won exchanges without carrying the motion's decisional test. Your sentence 1 must explicitly distinguish between the reality of the turn-level wins and their insufficiency for the motion. Do not imply the wins were illusory.`
        : "";
